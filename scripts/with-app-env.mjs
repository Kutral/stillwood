...
function main(argv) {
  ...
  // Windows: bare package-bin names resolve only via .cmd shims, which need shell mode.
  const needsShell = process.platform === "win32" && !/\.(cmd|ps1|bat|exe)$/i.test(command);
  const child = spawn(command, args, { stdio: "inherit", env, shell: needsShell });
